...
MODEL_DIR  = "models"
OUTPUT_DIR = os.path.join("outputs", "ocsvm")
...
CONFIG_PATH          = os.path.join(MODEL_DIR, "ocsvm_config.json")
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"[OCSVM] {MODEL_NAME} Degerlendirmesi")
    print(f"{'='*55}")
    ...
    print(f"\n[OCSVM] Accuracy  : {acc:.4f}")
    print(f"[OCSVM] Precision : {prec:.4f}")
    print(f"[OCSVM] Recall    : {rec:.4f}")
    print(f"[OCSVM] F1-Score  : {f1:.4f}")
    print(f"[OCSVM] ROC-AUC   : {roc:.4f}" if roc else "[OCSVM] ROC-AUC   : N/A")
