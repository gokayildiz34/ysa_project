...
CONFIG_PATH          = os.path.join(MODEL_DIR, "pca_config.json")
...
def plot_reconstruction_error_histogram(errors, threshold, y_true, out_dir):
    ...
    ax.hist(normal_errors,  bins=60, alpha=0.7, color="#00d4aa", label="Normal")
    ax.hist(anomaly_errors, bins=60, alpha=0.7, color="#ff4757", label="Anomaly")
    ax.axvline(threshold, color="#fbbf24", linestyle="--", linewidth=2, label=f"Threshold={threshold:.6f}")

    ax.set_xlabel("Reconstruction Error (MSE)", color="white")
    ax.set_ylabel("Count", color="white")
    ax.set_title(f"{MODEL_NAME} — Reconstruction Error Distribution", color="white", fontsize=13)
    ax.tick_params(colors="white")
    ax.spines[:].set_color("#1a2744")
    ax.legend(facecolor="#1e293b", edgecolor="#2a3a5e", labelcolor="white")

    path = os.path.join(out_dir, "reconstruction_error_histogram.png")
    plt.tight_layout()
    plt.savefig(path, dpi=120, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close()
    print(f"[KAYIT] {path}")
...
def plot_roc_curve(y_true, errors, roc_auc, out_dir):
...
def plot_explained_variance(model, out_dir):
    ...
    ax.bar(range(1, n+1), ratios * 100, color=MODEL_COLOR, alpha=0.7, label="Individual")
    ax2 = ax.twinx()
    ax2.plot(range(1, n+1), cumulative * 100, color="#fbbf24", linewidth=2, marker="o", markersize=4, label="Cumulative")
    ax2.set_ylim(0, 105)
    ax2.tick_params(colors="white")
    ax2.set_ylabel("Cumulative Variance (%)", color="white")
    ax2.spines[:].set_color("#1a2744")

    ax.set_xlabel("Principal Component", color="white")
    ax.set_ylabel("Explained Variance (%)", color="white")
    ax.set_title(f"{MODEL_NAME} — Explained Variance per Component", color="white", fontsize=13)
    ax.tick_params(colors="white")
    ax.spines[:].set_color("#1a2744")

    lines1, labels1 = ax.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(lines1+lines2, labels1+labels2, facecolor="#1e293b", edgecolor="#2a3a5e", labelcolor="white")
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"[PCA] {MODEL_NAME} Degerlendirmesi")
    print(f"{'='*55}")
    ...
    try:
        roc = float(roc_auc_score(y_true, errors))
    except Exception:
        roc = None

    print(f"\n[PCA] Accuracy  : {acc:.4f}")
    print(f"[PCA] Precision : {prec:.4f}")
    print(f"[PCA] Recall    : {rec:.4f}")
    print(f"[PCA] F1-Score  : {f1:.4f}")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 50 + "\n\n")
        f.write(f"n_components     : {model.n_components_}\n")
        f.write(f"Explained Var.   : {model.explained_variance_ratio_.sum()*100:.2f}%\n\n")
        if config:
            f.write("Config:\n")
    ...
    if roc is not None:
        plot_roc_curve(y_true, errors, roc, OUTPUT_DIR)
    plot_explained_variance(model, OUTPUT_DIR)
